...
class ThreeDElement extends React.Component {
    ...
    createCanvas() {
        ...
        let animate = function () {
            ...
            let newSize = Math.max(600, Math.min(canvasContainer.clientHeight, canvasContainer.clientWidth, 900)) - 100
    ...
    createCubesGeometry(points) {
        ...
        for (let i = 0; i < nops - 1; i += 1) {
            ...
            cube.position.set(points[i].x + Math.random() / 10.0, points[i].y, points[i].z)
            cubes.add(cube);
        }
        return cubes
    }
    ...
    render() {
        // return container
        return (
            <div id="canvas-container" ref={this.myRef}/>
        )
    }
