...
class AnimateComponent extends Rete.Component {
    ...
    worker(node, inputs, outputs) {
        this.min = inputs["min"].length ? inputs["min"][0] : parseFloat(node.data.min);
        this.max = inputs["max"].length ? inputs["max"][0] : parseFloat(node.data.max);
        this.delta = inputs["delta"].length ? inputs["delta"][0] : parseFloat(node.data.delta);
        ...
        let triProgress
        if (node.data.mirror) {
            triProgress = 0.5 + 1 / Math.PI * Math.asin(Math.sin(2 / b * Math.PI * (0.5 * x - b / 4)))
        } else {
            triProgress = (x % b) / b
        }
