...
export class CubeComponent extends Rete.Component {
  ...
  builder(node) {
    ...
    return node.addOutput(out1).addInput(inpRad).addInput(inpK);
  }
  ...
  worker(node, inputs, outputs) {
    ...
    outputs["v3"] = {
      x: function (i) {
        return (
          (r * Math.sin(i) * Math.cos(k * i)) /
          Math.pow(
            Math.pow(Math.sin(i), 6) *
              (Math.pow(Math.sin(k * i), 6) + Math.pow(Math.cos(k * i), 6)) +
              Math.pow(Math.cos(i), 6),
            1 / 6
          )
        );
      },
