...
export default class CustomComponent extends Rete.Component {
    ...
    builder(node) {
        ...
        const fy = new FunctionControl(this.editor, "fy", node, false, 't*floor(t)')
    ...
    worker(node, inputs, outputs) {
        ...
        const controls = this.editor.nodes
        ...
        try {
            ...
            fx = parse('f(t) =' + node.data.fx).compile().evaluate()
            fy = parse('f(t) =' + node.data.fy).compile().evaluate()
            fz = parse('f(t) =' + node.data.fz).compile().evaluate()
