...
export default class OutputComponent extends Rete.Component {
  ...
  builder(node) {
    ...
    const transformInput = new Rete.Input(
      "transform",
      "Transform",
      tranformSocket
    );
    ...
    deltaInput.addControl(
      new NumControl(this.editor, "delta", node, false, 0.005, 0.001)
    );
    nopsInput.addControl(
      new NumControl(this.editor, "nops", node, false, 2000, 50)
    );
    initialInput.addControl(
      new NumControl(this.editor, "initial", node, false, 0, 0.1)
    );
  ...
  worker(node, inputs, outputs) {
    ...
    const initial = inputs["initial"].length
      ? inputs["initial"][0]
      : node.data.initial;
    ...
    window.outputs[node.data.outputId] = {
      vector,
      style,
      delta,
      nops,
