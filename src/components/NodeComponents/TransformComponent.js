...
class TransformComponent extends Rete.Component {
  ...
  builder(node) {
    ...
    return node.addOutput(out1).addInput(rotX).addInput(rotY).addInput(rotZ);
  }

  worker(node, inputs, outputs) {
    const rotX = inputs["rot X"].length
      ? inputs["rot X"][0]
      : node.data["rot X"];
    const rotY = inputs["rot Y"].length
      ? inputs["rot Y"][0]
      : node.data["rot Y"];
    const rotZ = inputs["rot Z"].length
      ? inputs["rot Z"][0]
      : node.data["rot Z"];
